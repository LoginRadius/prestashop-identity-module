...
function loginRadiusGetSharingDiv($horizontal, $vertical)
{
    ...
    if (Configuration::get($horizontal) == 'on') {
        $show_horizontal = true;
    }

    if (Configuration::get($vertical) == 'on') {
        $show_vertical = true;
    }
...
function loginRadiusAddSharingDiv($show_horizontal, $show_vertical)
{
    ...
    if (Configuration::get('lr_enable_social_horizontal_sharing') == '1' && $show_horizontal) {

        if (Configuration::get('lr_chooseshare') != 8 && Configuration::get('lr_chooseshare') != 9) {
    ...
    if (Configuration::get('lr_enable_social_vertical_sharing') == '1' && $show_vertical) {
        if (Configuration::get('lr_chooseverticalshare') != 6 && Configuration::get('lr_chooseverticalshare') != 7) {
    ...
    return $module->display(__PS_BASE_URI__ . 'modules/loginradiusadvancemodule', 'sharing.tpl');
}
...
{
    $context = Context::getContext();
    $context->controller->addJS(__PS_BASE_URI__ . 'modules/loginradiusadvancemodule/views/js/sharinginterface.js');
    $share_script = '';
    $horizontal_theme = Configuration::get('lr_chooseshare') ? Configuration::get('lr_chooseshare') : '0';

    if ($horizontal_theme == 8 || $horizontal_theme == 9) {
        $counter_list = unserialize(Configuration::get('lr_socialshare_show_counter_list'));
        ...
        if (empty($lr_rearrange_settings)) {
            $lr_rearrange_settings = array('facebook', 'googleplus', 'twitter', 'linkedin', 'pinterest');
        }

        $providers = implode('","', $lr_rearrange_settings);
        $interface = 'horizontal';
        $size = '32';

        if ($horizontal_theme == 2 || $horizontal_theme == 3) {
            $interface = 'simpleimage';
        } else if ($horizontal_theme == 10) {
		...
		$u = LoginRadius.user_settings; ' . $sharecounttype . ' $u.isMobileFriendly=' . $mobileFriendly. '; $i.size = ' . $size . ';$i.show("lrsharecontainer"); });</script>';
    }
...
function loginRadiusVerticalShareScript()
{
    ...
    $context->controller->addJS(__PS_BASE_URI__ . 'modules/loginradiusadvancemodule/views/js/sharinginterface.js');
    $share_script = '';
    $vertical_theme = Configuration::get('lr_chooseverticalshare') ? Configuration::get('lr_chooseverticalshare') : '6';

    if ($vertical_theme == 6 || $vertical_theme == 7) {
        $counter_list = unserialize(Configuration::get('lr_socialshare_counter_list'));
        ...
        $lr_choosesharepos = Configuration::get('lr_choosesharepos');

        if ($lr_choosesharepos == 0) {
            $position1 = 'top';
            $position2 = 'left';
        } else if ($lr_choosesharepos == 1) {
            $position1 = 'top';
            $position2 = 'right';
        } else if ($lr_choosesharepos == 2) {
            $position1 = 'bottom';
            $position2 = 'left';
        } else {
            $position1 = 'bottom';
        ...
        if (empty($lr_vertical_rearrange_settings)) {
            $lr_vertical_rearrange_settings = array('facebook', 'googleplus', 'twitter', 'linkedin', 'pinterest');
        }

        $providers = implode('","', $lr_vertical_rearrange_settings);
